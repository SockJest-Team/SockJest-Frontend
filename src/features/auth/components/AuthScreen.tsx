...
function AuthForm() {
  ...
  const toggleMode = () => {
    const novoEstado = !isLogin;
    router.replace(`/auth?mode=${novoEstado ? "login" : "register"}`, {
      scroll: false,
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        queryClient.clear();
        login(
          { id: data.userId, nombre: form.name, email: form.email, roles: [] },
          data.access_token,
          data.refresh_token,
        );
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F6] text-stone-900 flex items-center justify-center p-6 selection:bg-stone-900 selection:text-white">
      <div className="w-full max-w-5xl bg-white border border-stone-200 shadow-xl grid grid-cols-1 md:grid-cols-2 overflow-hidden relative">
        <div className="p-10 md:p-14 flex flex-col justify-between bg-stone-900 text-white relative overflow-hidden min-h-120">
        ...
        <div className="p-10 md:p-14 flex flex-col justify-center bg-white relative min-h-130">
          <AnimatePresence mode="wait">
            {isLogin ? (
              <motion.form
                key="form-login"
                      ...
                      {rolesDisponibles.map((r) => {
                        ...
                        return (
                          <button
                            ...
                            className={`p-3 text-left border transition-all cursor-pointer flex flex-col justify-between ${activo ? "border-stone-900 bg-stone-900 text-white shadow-sm" : "border-stone-200 bg-stone-50 text-stone-800 hover:border-stone-400"}`}
                          >
